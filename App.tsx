...
const App: React.FC = () => {
  ...
  const handleReminderTimeout = useCallback(async () => {
    if (activeReminder) {
      const now = new Date();
      const dateKey = now.toDateString();
      const smsKey = `${activeReminder.id}_missed_${dateKey}`;
      
      // Prevent double sending of missed dose alert
      // 1. Check Ref Lock (Immediate blocking for rapid re-renders/race conditions)
      if (timeoutLock.current.has(smsKey)) {
        return;
      }

      // 2. Check State Persistence (Blocks subsequent attempts after successful send)
      if (lastSmsTime[smsKey]) {
          // Ensure log exists if somehow state says SMS sent but log missing
          addLog(activeReminder.id, 'missed', activeReminder.caretakerId);
          return;
      }

      // Lock it immediately
      timeoutLock.current.add(smsKey);
      
      // Update state to reflect attempt
      setLastSmsTime(prev => ({ ...prev, [smsKey]: Date.now() }));
  ...
  }, [activeReminder, lastSmsTime, allUsers]);
